...
def get_best_home_match_score(tile):
    if not HOME_TEMPLATES:
        return 0.0
...
def get_home_hsv_distance(tile):
    if "Home" not in HSV_REFERENCE:
        return 999.0
    ...
    tile_vector = np.array([h, s, v], dtype=float)

    home_mean = np.array(HSV_REFERENCE["Home"]["mean"], dtype=float)
    home_std = np.array(HSV_REFERENCE["Home"]["std"], dtype=float)

    home_std[home_std == 0] = 1.0

    distance = np.linalg.norm((tile_vector - home_mean) / home_std)
